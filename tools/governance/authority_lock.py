...
RECOVERY_LOCK_NAME = 'authority.lock.recovery'
...
def _recovery_mutex(root: Path) -> Iterator[None]:
    """Serialize stale recovery across local processes using an OS file lock."""
    base = governance_tmp_root(root.resolve()); base.mkdir(parents=True, exist_ok=True)
...
def current_owner(root: Path) -> dict | None:
    ...
    return data or {'task_id': None, 'pid': None}


def cleanup_stale(root: Path) -> bool:
    """Remove only the exact stale lock observed inside the recovery mutex."""
    root = root.resolve(); path = _lock_path(root)
    with _recovery_mutex(root):
        if not path.exists():
            return False
        owner = _read_owner(path); observed_identity = _identity(path); observed_instance = owner.get('lock_instance_id')
        try:
...
def acquire(root: Path, task_id: str, file: str) -> Path:
    ...
    try:
        ...
        if owner.get('task_id') == task_id:
            raise RuntimeError('AUTHORITY_LOCK_ALREADY_HELD') from exc
        raise RuntimeError('AUTHORITY_LOCK_BUSY') from exc
    try:
...
def release(root: Path, task_id: str) -> None:
    root = root.resolve(); validate_task_id(task_id); path = _lock_path(root)
    if not path.exists(): return
    owner = _read_owner(path)
    if owner.get('task_id') != task_id: raise RuntimeError('AUTHORITY_LOCK_NOT_OWNER')
    path.unlink()


def main() -> int:
    p = argparse.ArgumentParser(); sub = p.add_subparsers(dest='cmd', required=True)
    a = sub.add_parser('acquire'); a.add_argument('--root', default='.'); a.add_argument('--task-id', required=True); a.add_argument('--file', required=True)
    ...
    try:
        if args.cmd == 'acquire': print(acquire(root, args.task_id, args.file)); return 0
        if args.cmd == 'release': release(root, args.task_id); return 0
