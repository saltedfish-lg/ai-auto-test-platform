...
from .authority_index import authority_index_status, authority_preview, query_authority_result, refs_by_locators
...
def _compact_required_ref(root:Path,ref:dict[str,Any])->dict[str,Any]:
    ...
    return {
        ...
        'coverage_evidence_source':ref.get('coverage_evidence_source','PROJECTED_PRECISE_REF'),
        'sha256':file_sha256(source) if path and source.is_file() else 'MISSING',
    }


def _validated_task_authority_evidence(root:Path,ctx:dict[str,Any],routed_paths:list[str],index_state:dict[str,Any])->tuple[list[dict[str,Any]],list[dict[str,Any]]]:
    """Revalidate precise task-bound loads against the current Authority index."""
    history=ensure_context_history(ctx)
    consumer=context_consumer_id(ctx)
    task_id=str(ctx.get('task_id') or '')
    routed=set(routed_paths)
    terminal=str(ctx.get('task_status') or '') in {'SUCCESS','COMPLETED','FAILED','CANCELLED','ABORTED'}
    candidates=[]; invalid=[]
    for item in history.get('authority') or []:
        path=str(item.get('locator') or ''); selector=str(item.get('scope') or '')
        if path not in routed:
            continue
        evidence=item.get('authority_evidence')
        reason=None
        if str(item.get('consumer_id') or 'SINGLE_CONTINUOUS_CONTEXT_CONSUMER')!=consumer:
            reason='CONTEXT_CONSUMER_MISMATCH'
        elif terminal:
            reason='TASK_NOT_ACTIVE'
        elif not task_id or not isinstance(evidence,dict) or str(evidence.get('task_id') or '')!=task_id:
            reason='TASK_OWNERSHIP_NOT_VERIFIED'
        elif not bool(item.get('expanded')) or evidence.get('status')!='FULL_RECORD_LOADED':
            reason='FULL_RECORD_LOAD_NOT_VERIFIED'
        elif not selector or not selector.startswith('/'):
            reason='PRECISE_SELECTOR_REQUIRED'
        elif evidence.get('identity_kind') not in {'CANONICAL','STRUCTURAL','LOCATOR'}:
            reason='PRECISE_RECORD_IDENTITY_REQUIRED'
        elif evidence.get('identity_kind')!='LOCATOR' and not (evidence.get('canonical_record_id') or evidence.get('structural_id')):
            reason='PRECISE_RECORD_IDENTITY_REQUIRED'
        elif not evidence.get('record_sha256'):
            reason='RECORD_FINGERPRINT_REQUIRED'
        if reason:
            invalid.append({'path':path,'selector':selector,'reason':reason})
            continue
        candidates.append((item,evidence,f'{path}#{selector}'))
    current=refs_by_locators(root,(locator for _,_,locator in candidates),verified_index_state=index_state)
    valid=[]; seen=set()
    for item,evidence,locator in candidates:
        path=str(item.get('locator') or ''); selector=str(item.get('scope') or '')
        ref=current.get(locator)
        reason=None
        if ref is None:
            reason='CURRENT_AUTHORITY_RECORD_NOT_FOUND'
        elif evidence.get('canonical_record_id') and str(ref.get('canonical_record_id') or '')!=str(evidence.get('canonical_record_id')):
            reason='CANONICAL_IDENTITY_CHANGED'
        elif evidence.get('identity_kind')=='STRUCTURAL' and str(ref.get('structural_id') or '')!=str(evidence.get('structural_id') or ''):
            reason='STRUCTURAL_IDENTITY_CHANGED'
        elif evidence.get('identity_kind')=='LOCATOR' and ref.get('identity_kind')!='LOCATOR':
            reason='LOCATOR_IDENTITY_CHANGED'
        elif str(ref.get('record_sha256') or '')!=str(evidence.get('record_sha256') or ''):
            reason='AUTHORITY_RECORD_CHANGED'
        if reason:
            invalid.append({'path':path,'selector':selector,'canonical_record_id':evidence.get('canonical_record_id'),'reason':reason})
            continue
        key=_ref_key(ref)
        if key in seen:
            continue
        copied=dict(ref); copied['coverage_evidence_source']='TASK_LOADED_PRECISE_EVIDENCE'
        valid.append(copied); seen.add(key)
    return valid,invalid


def _required_authority_refs(root:Path,refs:list[dict[str,Any]],relationship:dict[str,Any],routed_paths:list[str],history_refs:list[dict[str,Any]])->tuple[list[dict[str,Any]],dict[str,Any]]:
    ...
    """
    projected={_ref_key(ref):{**ref,'coverage_evidence_source':'PROJECTED_PRECISE_REF'} for ref in refs if ref.get('identity_kind')!='ROUTED_FILE_REF' and ref.get('selector')}
    historical={_ref_key(ref):ref for ref in history_refs}
    by_key={**historical,**projected}
    ...
    coverage_evidence=[]
    loaded_core_paths:set[str]=set()
    for path in routed_paths:
        candidate=next((ref for ref in projected.values() if str(ref.get('path') or '')==path),None)
        if candidate is None:
            candidate=next((ref for ref in history_refs if str(ref.get('path') or '')==path),None)
        if candidate is None:
            missing_core_authorities.append(path)
            coverage_evidence.append({'path':path,'status':'UNCOVERED','evidence_source':'ROUTED_FILE_REF_PENDING_EXPANSION'})
            continue
        key=_ref_key(candidate); core_coverage_keys.add(key); required_keys.add(key)
        loaded_candidate=next((ref for ref in history_refs if _ref_key(ref)==key),None)
        if loaded_candidate is not None:
            loaded_core_paths.add(path)
        coverage_evidence.append({'path':path,'selector':candidate.get('selector'),'canonical_record_id':candidate.get('canonical_record_id'),'structural_id':candidate.get('structural_id'),'status':'COVERED' if loaded_candidate is not None else 'LOCATED_PENDING_FULL_LOAD','evidence_source':candidate.get('coverage_evidence_source')})
    ...
    coverage={
        ...
        'represented_core_authority_count':len(set(routed_paths)-set(missing_core_authorities)),
        ...
        'current_required_authority_count':len(set(routed_paths)),
        'current_covered_authority_count':len(loaded_core_paths),
        'coverage_evidence':coverage_evidence,
        'uncovered_requirements':[{'path':path,'reason':'PRECISE_AUTHORITY_EVIDENCE_REQUIRED'} for path in sorted(set(routed_paths)-loaded_core_paths)],
        'complete':not bool(missing_core_authorities),
    }
    return required,coverage


def _loaded_required_authority_refs(required:list[dict[str,Any]],history_refs:list[dict[str,Any]])->tuple[list[dict[str,Any]],list[dict[str,Any]]]:
    loaded_keys={_ref_key(ref) for ref in history_refs}
    loaded=[]; missing=[]
    for ref in required:
        if _ref_key(ref) in loaded_keys: loaded.append(ref)
...
def enrich_task_context(root:Path,ctx:dict[str,Any])->dict[str,Any]:
    ...
    history_refs,stale_evidence=_validated_task_authority_evidence(root,out,authorities,index_state) if index_state['status']=='READY' else ([],[])
    required_refs,required_fact_coverage=_required_authority_refs(root,refs,relationship,authorities,history_refs)
    loaded_refs,missing_required_refs=_loaded_required_authority_refs(required_refs,history_refs)
    ...
    elif index_state['status'] in {'MISSING','STALE','INVALID','PARTIAL'} or query.get('status')=='PARTIAL' or (query.get('status')=='TRUNCATED' and (core_authority_coverage_incomplete or required_facts_not_loaded or relationship_incomplete)) or unrepresented or relationship_incomplete or core_authority_coverage_incomplete or required_facts_not_loaded or preview_incomplete:
    ...
    fallback_file_refs=[{'path':ref.get('path'),'evidence_source':'ROUTED_FILE_REF_PENDING_EXPANSION'} for ref in refs if ref.get('identity_kind')=='ROUTED_FILE_REF']
    out['coverage_evidence']=required_fact_coverage.get('coverage_evidence') or []
    out['uncovered_authority_requirements']=required_fact_coverage.get('uncovered_requirements') or []
    out['stale_authority_evidence']=stale_evidence
    out['fallback_authority_file_refs']=fallback_file_refs
    out['authority_slice']={**preview,'query_status':query.get('status'),'query_diagnostics':diagnostics,'authority_role_strategy':'ALL_ROUTED_CONSERVATIVE','routed_core_authorities':sorted(set(authorities)),'routed_supporting_authorities':[],'unrepresented_routed_authorities':unrepresented,'direct_read_required':unavailable_sources,'fallback_file_refs':fallback_file_refs}
