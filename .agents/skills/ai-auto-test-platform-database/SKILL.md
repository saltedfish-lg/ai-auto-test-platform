...
description: AI自动化测试执行平台 MySQL 8.4、冻结DDL、SQLAlchemy映射、事务、约束、索引与Migration执行Skill。
---
...
- 正式数据库：MySQL 8.4；
- R4.2 P1 顺序：V3 → V4 → V5；
- V3/V4/V5 位于 `docs/baseline/R4.2/编码冻结基线/DATABASE_DDL/`，冻结只读；
- admin bootstrap 在 Migration + RBAC Seed 之后独立执行；
- 不允许用 extension JSON 承载正式契约字段。

## 编码规则

- SQLAlchemy 映射服从冻结 DDL；
- DB 唯一键/FK/CHECK/row version 要真实生效；
- 事务边界服从业务原子性；
- P1 已有 V5 时不得再自创一套认证表；
- 索引、批量查询、Repository 组织等纯工程优化可自主决定，但不能改变冻结唯一性、生命周期、并发语义；
