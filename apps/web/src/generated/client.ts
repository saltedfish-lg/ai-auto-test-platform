...
export class ApiClient {
  ...
  async list_runner(options: QueryRequestOptions<{ page?: number; page_size?: number; sort?: string; filter?: string }> = {}): Promise<ListRunnerResponse> {
  ...
  async list_runner_capability(options: QueryRequestOptions<{ page?: number; page_size?: number; sort?: string; filter?: string }> = {}): Promise<ListRunnerCapabilityResponse> {
    let path = "/api/v1/runner-capability";
    const query = new URLSearchParams();
    if (options.query?.page !== undefined) query.set("page", String(options.query?.page));
    if (options.query?.page_size !== undefined) query.set("page_size", String(options.query?.page_size));
    if (options.query?.sort !== undefined) query.set("sort", String(options.query?.sort));
  ...
  async get_runner_capability(id: string, options: RequestOptions = {}): Promise<GetRunnerCapabilityResponse> {
    let path = "/api/v1/runner-capability/{id}".replace('{id}', encodeURIComponent(id));
