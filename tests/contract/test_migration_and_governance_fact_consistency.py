...
def test_runtime_gate_catalog_is_definition_only():
 d=yaml.safe_load((AUTH/'编码权威事实/SYSTEM_DESIGN.yaml').read_text(encoding='utf-8')); ids={x['gate_id'] for x in d['runtime_gate_catalog']['gates']}; assert {'AUTH_MYSQL_RUNTIME_GATE','AUTH_BROWSER_RUNTIME_GATE','FULL_SCHEMA_MYSQL84_RUNTIME_GATE','FLYWAY_MIGRATION_RUNTIME_GATE'}.issubset(ids); assert all('status' not in x for x in d['runtime_gate_catalog']['gates'])
