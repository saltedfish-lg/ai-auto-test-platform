...
def test_current_runtime_contract_boundaries_use_r4_2() -> None:
    sources = (ROOT / "packages/contracts/src/platform_contracts/sources.py").read_text(
        encoding="utf-8"
    )
...
def test_p1_traceability_uses_current_r4_2_and_archives_r4_1_blocker() -> None:
    current = (ROOT / "docs/implementation/p1-auth-rbac-traceability.md").read_text(
        encoding="utf-8"
    )
    ...
    assert "历史证据，禁止作为当前P1实现事实源" in historical  # noqa: RUF001
