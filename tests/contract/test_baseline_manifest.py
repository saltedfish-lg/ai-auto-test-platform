...
def test_current_manifest_and_navigation_are_consistent() -> None:
    report = verify()

    assert report["status"] == "PASS"
    assert report["manifest_entries"] == 724
    assert (ROOT / "docs" / "baseline" / "CURRENT").read_text(encoding="utf-8").strip() == "R4.2"
