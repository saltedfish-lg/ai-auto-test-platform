...
def run_local_mysql(root: Path, mysql: Path, host: str, port: str, user: str, password: str, report: dict) -> int:
    ...
    databases = [f"atp_r42_empty_{suffix}", f"atp_r42_upgrade_{suffix}"]
    ...
    ddl = root / "编码冻结基线/DATABASE_DDL"
    validation = root / "编码冻结基线/RELEASE/validation"
    ...
    try:
        ...
        execute("UPGRADE_R41_V3", v3, databases[1])
        execute("UPGRADE_R41_V4", v4, databases[1])
        execute("UPGRADE_R42_V5", v5, databases[1])
        upgrade_assert = execute("UPGRADE_ASSERTIONS", assertions, databases[1]).stdout
        if "R4_2_MYSQL84_GATE_PASS" not in empty_assert or "R4_2_MYSQL84_GATE_PASS" not in upgrade_assert:
