...
BEGIN
  ...
  IF (SELECT COUNT(*) FROM information_schema.tables WHERE table_schema = DATABASE() AND table_type='BASE TABLE') <> 84 THEN
    SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT='Expected exactly 84 base tables';
  END IF;
  IF EXISTS (
...
DELIMITER ;
SELECT 'R4_2_MYSQL84_GATE_PASS' AS result;
