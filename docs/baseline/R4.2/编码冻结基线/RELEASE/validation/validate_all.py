...
def main():
    ...
    v.add("GOVERNANCE_AND_ARCHITECTURE",not meta_errors,"R4.2 metadata and frozen architecture",meta_errors)
    ...
    # 5 DDL and schema
    sql_rels=["编码冻结基线/DATABASE_DDL/V3__platform_contract_rebuild.sql",
              "编码冻结基线/DATABASE_DDL/V5__platform_authentication_contract.sql"]
    sql="\n".join((args.root/rel).read_text(encoding="utf-8") for rel in sql_rels)
    tables=parse_create_tables(sql); fks=parse_fks(sql)
    ddl_errors=[]
    if len(tables)!=84:ddl_errors.append(f"CREATE TABLE count={len(tables)}")
    # columns and PK
    ...
    if len(schema_tables)!=84:ddl_errors.append(f"schema tables={len(schema_tables)}")
    ...
    if contract_rules.get("release_id")!=RELEASE_ID:
        api_errors.append(f"x-contract-rules release_id={contract_rules.get('release_id')}")
