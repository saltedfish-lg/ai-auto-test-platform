...
RID='PDBR-2026.08.07-R4.2'; AUTH='AUTHORITY-MODEL-R4.2-001'
CORE=[
...
def main():
 ...
 # Current package member metadata must point to R4.2; earlier releases are allowed only as explicit parent/effective/historical provenance.
 meta_bad=[]
 domain=texts[CORE[2]]
 ...
 for r in current_contracts:
  ...
  if 'parent_release_id: PDBR-2026.08.06-R4.1' not in head: meta_bad.append(f'{r}:parent release metadata')
 ...
 if accm.get('release_id')!=RID or accm.get('parent_release_id')!='PDBR-2026.08.06-R4.1': meta_bad.append('编码冻结基线/ACCEPTANCE_CLOSURE/acceptance-closure.json:release chain')
 ...
 for marker in ['blocking_scope: DATABASE_MODULE_FORMAL_MERGE','status: PASS','blocks_code_initialization: false','blocks_database_module_formal_merge: false','blocking_scope: IMPLEMENTATION_RELEASE_READINESS','blocks_production_release: true']:
