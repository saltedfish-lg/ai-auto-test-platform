...
export class ApiClient {
  ...
  async logout_platform_user(body: AuthCookieActionRequest, options: RequestOptions = {}): Promise<unknown> {
    const path = "/api/v1/auth/logout";
    return this.request<unknown>(path, { method: 'POST', headers: { 'Content-Type': 'application/json', ...(options.headers ?? {}) }, signal: options.signal, body: JSON.stringify(body) });
  }
  ...
  async change_current_user_password(body: ChangePasswordRequest, options: RequestOptions = {}): Promise<AuthenticationResponse> {
