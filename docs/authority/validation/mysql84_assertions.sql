-- Current Living Authority MySQL 8.4 V3 → V4 → V5 → V6 → V7 → V8 → V9 → V10 → V11 → V12 → V13 → V14 → V15 → V16 → V17 → V18 → V19 → V20 runtime assertions
DELIMITER //
...
BEGIN
  ...
  IF (SELECT COUNT(*) FROM information_schema.tables WHERE table_schema = DATABASE() AND table_type='BASE TABLE') <> 105 THEN
    SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT='Expected exactly 105 base tables after V3 → V4 → V5 → V6 → V7 → V8 → V9 → V10 → V11 → V12 → V13 → V14 → V15 → V16 → V17 → V18 → V19 → V20';
  END IF;
  IF EXISTS (
