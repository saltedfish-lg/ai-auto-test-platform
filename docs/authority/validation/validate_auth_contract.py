...
def main() -> int:
    ...
    add(
        ...
        and auth["metadata"].get("deferred_product_decisions") == 3
        and auth["metadata"].get("implementation_status") == AUTH_IMPLEMENTATION_STATUS,
        (
            ...
            f"implementation={auth['metadata'].get('implementation_status')}"
        ),
    )
    ...
    if set(decision_index) != set(DEFERRED_PRODUCT_DECISIONS):
    ...
    for decision_id, expected_name in DEFERRED_PRODUCT_DECISIONS.items():
        ...
        if item.get("name") != expected_name:
            decision_errors.append(f"{decision_id}:name={item.get('name')}")
        ...
        if item.get("decision_owner") != "USER_PRODUCT_SOVEREIGNTY":
            decision_errors.append(f"{decision_id}:owner={item.get('decision_owner')}")
        if item.get("blocks_current_approved_scope") is not False:
            decision_errors.append(
                f"{decision_id}:blocks_current_approved_scope={item.get('blocks_current_approved_scope')}"
            )
        if not item.get("current_fact") or not item.get("missing_facts") or not item.get("blocked_scope") or not item.get("implementation_rule"):
            decision_errors.append(f"{decision_id}:placeholder-incomplete")
    add(
        "AUTH-DEFERRED-PRODUCT-DECISIONS",
        not decision_errors,
        (
            "GOV-P1-002/003/005 remain explicitly deferred and non-blocking for the approved P1 Auth scope; "
    ...
    add(
        ...
        auth["metadata"].get("implementation_status") == AUTH_IMPLEMENTATION_STATUS
        and not missing_implementation_files,
        f"implementation_status={auth['metadata'].get('implementation_status')}; missing={missing_implementation_files}",
    )
    add(
    ...
    api = yaml.safe_load((root / "编码权威事实/OPENAPI/openapi.yaml").read_text(encoding="utf-8"))
    ...
    schema = yaml.safe_load((root / "编码权威事实/DATABASE_DDL/database-schema.yaml").read_text(encoding="utf-8"))
    ...
    owner_registry = yaml.safe_load((root / "编码权威事实/STATE_OWNER_REGISTRY/state-owner-registry.yaml").read_text(encoding="utf-8"))
    ...
    add(
        ...
        len(owners) == 7 and len(semantic_ids) == len(set(semantic_ids)) and len(persistence_fields) == len(set(persistence_fields)),
        f"owners={len(owners)}; fields={len(persistence_fields)}",
    )

    permission = yaml.safe_load((root / "编码权威事实/PERMISSION_CLOSURE/permission-closure.yaml").read_text(encoding="utf-8"))
    ...
    system_design = yaml.safe_load((root / "编码权威事实/SYSTEM_DESIGN.yaml").read_text(encoding="utf-8"))
    add(
        "AUTH-CURRENT-AUTHORITY-READY",
        system_design.get("release_gate", {}).get("code_baseline_readiness", {}).get("status") == "READY_FOR_P1_IMPLEMENTATION"
        and auth["metadata"]["pending_user_decisions"] == 0,
