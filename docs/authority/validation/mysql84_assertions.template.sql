-- Current Living Authority MySQL 8.4 {{MIGRATION_CHAIN}} runtime assertions
DELIMITER //
...
BEGIN
  ...
  IF (SELECT COUNT(*) FROM information_schema.tables WHERE table_schema = DATABASE() AND table_type='BASE TABLE') <> {{TABLE_COUNT}} THEN
    SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT='Expected exactly {{TABLE_COUNT}} base tables after {{MIGRATION_CHAIN}}';
  END IF;
  IF EXISTS (
  ...
  END IF;
  IF (SELECT COUNT(*) FROM atp_permission_code) <> {{PERMISSION_COUNT}} THEN
    SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT='Permission seed count mismatch';
  END IF;
  IF (SELECT COUNT(*) FROM atp_role) <> {{ROLE_COUNT}} THEN
    SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT='Role seed count mismatch';
  END IF;
  IF (SELECT COUNT(*) FROM atp_role_permission) <> {{RBAC_MAPPING_COUNT}} THEN
